...
Guidance and configuration for **Google Antigravity (AGY / Gemini CLI)** agents in this repository. This is a **map and index** — each topic below links to its single source of truth. Don't duplicate that content here.
...
- **`.agents/`** — agents, skills, and configuration (see `.agents/` and `.claude/`).
...
| Code-level standards (auto-loaded skills) | `.agents/skills/{spring-boot-standards,angular-standards,api-design}` |
...
Cross-platform **Node hooks** in `.claude/` / `.agents/`: `block-prod-commands.mjs` (Bash), `secret-scan.mjs` (Write/Edit), `format.mjs` (PostToolUse), `run-tests.mjs` (Stop). Permissions deny `git push`, `rm -rf`, `sudo`, and reads/writes of `.env*` / `application-prod.*`. Enforcement beyond local agents: `.githooks/pre-push`, CI workflows, and branch protection (`scripts/setup-branch-protection.sh` + `.github/CODEOWNERS`).

## MCP integrations (optional)

Standard Model Context Protocol (MCP) integrations:
- SonarCloud MCP (`sonarcloud`) for code quality, security hotspots, coverage, and static analysis inspection (`.agents/mcp_config.json`).
