...
| **`nexus.rbac.role_became_fully_admin_equivalent{holders}`** *(new — US-017 D22/RC-15.3)* | Counter | Same `attachPermission` call, dangerous path only, **after** the attach — one further bounded read (`findPermissionNamesForRole`, M7) checking whether the attach was the **exact** one that tips the role from not-fully-admin-equivalent to fully-admin-equivalent (`RbacAdminEquivalence.isFullyAdminEquivalent`) | Extends the row above rather than replacing it — **same path** (`attachPermission`'s dangerous branch), **same admin-only reachability**, but a materially more serious question: the row above answers *"how many users did this attach silently escalate?"*; this one answers *"did this attach just make the role a **caller-side-qualifying administrator**?"* — a fact three separate `dangerous_permission_granted` increments, minutes or weeks apart, do not compose into for a human. Bounded `holders` bucket, reusing the same bucketing as the row above. **Cross-reference to RES-1(b), stated in both directions (§9.2's instruction):** the way a tenant acquires a dangerous role held by many users **is** US-016 RES-1(b)'s own step 3 (a `user:write` holder self-assigns a benign custom role today, legitimately; an administrator attaches a dangerous permission to it tomorrow); this counter is the moment that pre-positioned exposure becomes externally visible. Conversely, RES-1(b)'s entry (`docs/features/US-016/03-design.md` §12.3, cited by `docs/features/US-017/03-design.md` §12.3) now points back at this signal as its detection. Prometheus name: `nexus_rbac_role_became_fully_admin_equivalent_total` |
...
| **`nexus_rbac_role_became_fully_admin_equivalent`** *(new — US-017 D22/RC-15.3)* | `increase(nexus_rbac_role_became_fully_admin_equivalent_total[15m]) > 0` | **ticket; escalate to page when `holders != "0"`** | A role just crossed the ALL-three-dangerous-permissions threshold (`RbacAdminEquivalence.isFullyAdminEquivalent`) — it is now a full stand-in for `TENANT_ADMIN` on US-017's caller-side gate, not merely admin-equivalent on the target side. **`holders != "0"` is the exact signature of a mass silent promotion to administrator** — every existing active holder of that role became a caller-qualifying administrator in the same instant, with no self-assignment, no gate evaluation and no notice to them or to the grantor. Identify the role and grantor from the `RBAC_ROLE_BECAME_FULLY_ADMIN_EQUIVALENT` WARN (§4), then list the holders the same way as the row above. **Read together with `nexus_rbac_dangerous_permission_granted_to_holders`, not instead of it:** that alert may have already fired once or twice for the same role (first and second dangerous-permission attaches, each with existing holders); this alert is the one that says the *third* attach was the one that mattered. |
...
| **Role became fully admin-equivalent** *(new — US-017 D22/RC-15.3)* | `RoleManagementService.attachPermission`, dangerous path only, **after** the attach, when one bounded M7 read shows `RbacAdminEquivalence.isFullyAdminEquivalent` has just become `true` for this role | **WARN** | `event=RBAC_ROLE_BECAME_FULLY_ADMIN_EQUIVALENT`, `tenantId`, `roleId`, `roleName`, `holderCount`, `grantedBy`. Extends the row above rather than duplicating it: that WARN answers "how many holders did this attach silently escalate"; this one answers "did this attach just make the role a caller-side-qualifying administrator" — the moment US-016 **RES-1(b)**'s standing pre-positioning primitive (`docs/features/US-016/03-design.md` §12.3) becomes externally visible for this role, in either direction: RES-1(b) is this signal's cause, and this signal is RES-1(b)'s detection. Alertable via `nexus_rbac_role_became_fully_admin_equivalent` (§2) |
