...
>
> **[US-017, 2026-09-24] Closed end to end.** `docs/features/US-017/03-design.md` extends both halves named above to every admin-equivalent role, using the **same** `RbacAdminEquivalence.isAdminEquivalent` predicate for both (FR-1's tenant-wide, distinct-holder lockout guard, D5; and FR-2's widened `RbacZeroActiveAdminsHealthIndicator`, D10–D12) — so the preventive and detective controls cover the same population by construction, not by two independently-maintained queries. See US-017 `03-design.md` §12.3's own RES-3 citation for the corresponding entry there.
...
**[US-017, 2026-09-24] No longer true — recorded here rather than left to mislead.** `RbacZeroActiveAdminsHealthIndicator` **does** change in US-017 (FR-2, D10–D12/D25): it now detects the same admin-equivalent population `isAdminEquivalent` protects (the literal name **or** any `RbacDangerousPermissions` member), not the literal name alone, and injects a narrow `ZeroAdminTenantReader` rather than `JpaUserRoleRepository` directly. RES-3 is closed by that change; see `docs/features/US-017/03-design.md` D10–D12/D25 and §12.3. `RbacDbPrivilegeHealthIndicator` is unaffected by US-017.
...
| **RES-3** | AC5 lockout and `RbacZeroActiveAdminsHealthIndicator` cover only the literally-named `TENANT_ADMIN`. | Med | Out of scope (Gate 1 #8). This design strengthens the case (§6.4 corollary). Backlog story is a **merge-checklist item**. **[T-023, 2026-09-16] Discharged: filed as `docs/story/2-rbac/US-017.md`, paired with RES-9 as one Epic-3 question per US-017's own Background section.** **[US-017, 2026-09-24] Closed end to end:** FR-1 (the tenant-wide, distinct-holder lockout) and FR-2 (the widened health indicator) both use the single `RbacAdminEquivalence.isAdminEquivalent` predicate (US-017 D1/D5/D10–D12), so the preventive and detective controls cover the same population by construction. See `docs/features/US-017/03-design.md` §12.3. |
...
| **RES-9** *(new, T-E26)* | **The caller-side admin test remains name-based** while the target-side test becomes privilege-based. A user holding a custom role with all three dangerous permissions is therefore *subject to* the new gate but can never *pass* it — an operational dead end that pressures tenants toward granting literal `TENANT_ADMIN`. | Low | Accepted, deliberate, not an oversight. **Consistent with US-015 AC11**, which gates the mint side on the same literal role — US-016 inherits the asymmetry rather than introducing it. Making the caller-side test privilege-based is a materially larger decision (it would let a custom-role holder administer privileged assignments) and belongs with RES-3 as one Epic-3 question. Recorded in ADR-0017's follow-on rules (§13.2). **[US-017, 2026-09-24] Closed for the assign/revoke caller test only — not fully closed, and RES-13 carries the remainder forward; do not read this as fully discharged.** `docs/features/US-017/03-design.md` D2 adds a privilege-based caller-side predicate (`RbacAdminEquivalence.isFullyAdminEquivalent` — literal `TENANT_ADMIN` **or** ALL THREE dangerous permissions) to `assign()`/`revoke()`, so a holder of such a role can now pass the gate this row describes. **The mint side is explicitly not closed:** `RoleManagementService.attachPermission`/`detachPermission`'s caller test stays name-based (US-017 D18), so that remainder is carried forward as **US-017 RES-13**, per RC-8.2 discipline — this entry's cross-reference to RES-13 must not be dropped in any future edit. |
| **RES-10** *(new, T-D12, inherited)* | **Pre-existing cross-method lock cycle:** `assign(TENANT_ADMIN)` (S on the caller's row, then insert-intention in the `role_id = adminRoleId` gap) versus `revoke(TENANT_ADMIN)` (M1's next-key range lock) can cycle **today**, unchanged by this story. | Low | Accepted as inherited. Named in §7.2 property 3 and in harness C's Javadoc **so that a mixed-workload failure is not misattributed to US-016**. Filed as a separate backlog observation, **not** a US-016 fix. **[T-023, 2026-09-16] Discharged: filed as `docs/story/2-rbac/EPIC-002.md` Open Decisions item 8 — an observation, no successor story.** **[US-017, 2026-09-24] Not closed — under active investigation, do not treat as discharged.** `docs/features/US-017/03-design.md` D7 makes the acquisition order total across both verbs specifically to close this at the root, and the design's own exit gate is `LastAdminLockoutIT`'s reshaped harness C (the `assign(TENANT_ADMIN)` thread restored, plus a new benign thread) green over ≥5 consecutive runs under Docker. **As of the most recent Docker-backed run, that harness is failing** (`should_completeWithoutDeadlock_when_mixedPrivilegedRoleChangesRaceAcrossBothVerbs` times out) — a fix is in progress. Per the threat model's own language, until that evidence exists this entry stays **open**, not conditionally closed; see `docs/story/2-rbac/EPIC-002.md`'s merge checklist for current status. **[US-017, 2026-09-24] Closed — root cause was not a lock cycle.** Diagnosis: MC-C's own fix (native `FORCE INDEX (fk_user_roles_role)` on M11, `JpaUserRoleRepository`) surfaced an unrelated, pre-existing artifact — Spring Boot's Hikari default (`maximum-pool-size=10`) is one short of harness C's 11 truly-concurrent (`CyclicBarrier`-released) threads, so one thread always queued for the full 30 s Hikari connection-acquisition timeout, which alone exceeds the harness's 15 s `awaitTermination` budget regardless of locking correctness — confirmed by isolating a single diagnostic run with a 90 s budget, which completed cleanly in ~34 s and logged exactly this `HikariPool-1 ... request timed out after 30003ms (total=10, active=10, idle=0, waiting=8)` condition. Fix: `LastAdminLockoutIT`'s `@SpringBootTest` properties raise `spring.datasource.hikari.maximum-pool-size=15`, test-scoped only; the harness's original 15 s `awaitTermination` budget (the actual deadlock-detection sensitivity) was left unchanged. **Evidence: 5/5 consecutive green runs** against real Docker/Testcontainers MySQL 8.4 on 2026-09-24, zero exceptions and zero pool-timeout warnings in any run. D7's total-ordering fix is confirmed correct; the harness's earlier failures were a resource-sizing artifact, not evidence against D7. |
