...
**Mandatory caveat before you do anything else, quoted from the design (§9.3(b), RC-11.2):** *"this canary can only ever detect a bypass **inside** the gate's own logic — M5 answering the wrong question (T-E22's two fail-open axes, or an M8 resolution bug). It cannot detect a bypass **around** the gate,* because on the success path `privileged="true"` implies the gate ran and passed."* Do not treat a *non*-firing canary as proof the gate cannot be bypassed by some new code path that skips `RoleAssignmentService` entirely — it says nothing about that.

**Fixed per M-2 (`docs/features/US-016/07-security-review.md`), superseding the original "control-flow-inferred, unreachable `false`" caveat that used to live here:** `callerIsAdmin` is now independently re-derived from `callerHoldsActiveTenantAdmin(actor)` — a non-locking M4 projection over the caller's own active role names — rather than from `privileged` (the gate's own control-flow variable). It disagrees with M5's locking read (`hasActiveAdminAssignment`) if and only if the two mechanisms produce different answers about the caller's admin status, which is exactly what a T-E22-class fail-open bug in M5 would look like. `callerIsAdmin="false"` on the `privileged="true"` series is therefore a **genuinely reachable, meaningful signal** now, not an inert tag — treat a firing the same way as before (maximally anomalous, first response step below unchanged), but do not assume, as the old caveat did, that the tagging code itself cannot be trusted to ever show `false`.
