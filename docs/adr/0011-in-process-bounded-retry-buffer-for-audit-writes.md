...
**This is recorded here, not silently assumed, because no EPIC-007 planning document exists yet in this repository** (verified — no `EPIC-007` artifacts found under `docs/` or `docs/story/` at the time of writing) to serve as its natural home. Until EPIC-007's own planning docs exist, **this ADR is the authoritative tracking location** for the obligation below; EPIC-007's discovery/requirements phase MUST carry this forward (and may then supersede this note with its own ADR/design reference).
