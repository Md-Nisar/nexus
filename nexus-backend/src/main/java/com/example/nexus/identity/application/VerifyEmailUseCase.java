...
public class VerifyEmailUseCase {
  ...
  /**
   * Consumes a verification token and transitions the associated user from PENDING to ACTIVE.
   * Marks the token as consumed in the database, preventing replay attacks.
   *
   * @param rawToken     the raw verification token (64-char hex string generated by {@link TokenGenerator#generate()})
   * @param ctx          per-request context (IP, traceId, user-agent)
   * @throws TokenExpiredException if the token does not exist, has expired, is already consumed, or user state is invalid
   */
